...
import json, os, datetime, threading, time, random, string, requests, base64, re, subprocess, sys, html
...
def create_fakelag_key(product):
    ...
    try:
        data = response.json()
        key = str(data.get("key") or data.get("data") or data.get("result") or "").strip() if isinstance(data, dict) else ""
    except ValueError:
        key = raw
    if not key or "<html" in key.lower() or "<!doctype" in key.lower():
        raise RuntimeError("API không trả về key hợp lệ")
    return key
...
def cb_api_buy(call):
    uid = call.from_user.id
    if is_user_banned(uid) or is_user_blocked(uid):
        bot.answer_callback_query(call.id, "🚫 Tài khoản bị hạn chế!", show_alert=True)
        return
    ...
    with _lock:
        ...
        if int(u.get("balance", 0)) < price:
            bot.answer_callback_query(call.id, f"❌ Cần {fmt(price)}, bạn có {fmt(u.get('balance', 0))}.", show_alert=True); return
    bot.answer_callback_query(call.id, "⏳ Đang tạo key...")
    try:
        key = create_fakelag_key(p)
    except Exception as e:
        bot.send_message(uid, f"❌ <b>Không tạo được key.</b>\n<code>{html.escape(str(e)[:500])}</code>", parse_mode="HTML"); return
    exp = expires_at_days(int(p.get("days", 1)))
    with _lock:
        d = load(); u = get_user(d, uid)
        if int(u.get("balance", 0)) < price:
            bot.send_message(uid, "❌ Số dư vừa thay đổi, không thể hoàn tất đơn."); return
        d["ord_counter"] = int(d.get("ord_counter", 0)) + 1
        oid = mixed_id(d, "API", length=7)
        u["balance"] -= price
        order = {"id":oid,"uid":str(uid),"name":call.from_user.full_name,"username":call.from_user.username or "","pid":pid,"product":p.get("name","Sản phẩm"),"price":price,"key":key,"days":int(p.get("days",1)),"created_at":now(),"expires_at":exp,"status":"active"}
        d.setdefault("orders", []).append(order); u.setdefault("orders", []).append(oid); save(d); balance=u["balance"]
    text=(f"<b>✅ MUA HÀNG THÀNH CÔNG!</b>\n━━━━━━━━━━━━━━━━━━\n🎮 <b>{html.escape(str(p.get('name','Sản phẩm')))}</b>\n🧾 Mã đơn: <code>{html.escape(oid)}</code>\n🔐 Key: <code>{html.escape(key)}</code>\n📅 Hạn: <b>{html.escape(exp.replace('T',' '))}</b>\n💵 Giá: <b>{fmt(price)}</b>\n👜 Số dư còn: <b>{fmt(balance)}</b>\n━━━━━━━━━━━━━━━━━━\n⚠️ Hãy lưu lại Key. Bạn có thể xem lại tại 📦 Đơn của tôi.")
    markup=types.InlineKeyboardMarkup(row_width=2); markup.add(types.InlineKeyboardButton("🛒 Mua thêm",callback_data="SHOP"),types.InlineKeyboardButton("📦 Đơn của tôi",callback_data="MY_ORDERS"))
    try: bot.edit_message_text(text,call.message.chat.id,call.message.message_id,parse_mode="HTML",reply_markup=markup)
    except Exception: bot.send_message(uid,text,parse_mode="HTML",reply_markup=markup)
    for aid in get_admin_list():
        try: bot.send_message(aid,f"🛒 <b>ĐƠN MUA API MỚI</b>\n👤 {html.escape(call.from_user.full_name)}\n🆔 User: <code>{uid}</code>\n🛍 {html.escape(str(p.get('name','Sản phẩm')))}\n🧾 Đơn: <code>{html.escape(oid)}</code>\n💵 {fmt(price)}",parse_mode="HTML")
...
def cb_buy(call):
    ...
    success_text=(f"<b>✅ MUA HÀNG THÀNH CÔNG!</b>\n━━━━━━━━━━━━━━━━━━\n🎮 <b>{html.escape(str(p.get('name','Sản phẩm')))}</b>\n🧾 Mã đơn: <code>{html.escape(oid)}</code>\n💵 Đã trừ: <b>{fmt(p['price'])}</b>\n👜 Số dư còn: <b>{fmt(u['balance'])}</b>\n📧 Gmail: <code>{html.escape(gmail)}</code>\n🔑 Mật khẩu: <code>{html.escape(password)}</code>\n🕐 {html.escape(now())}\n━━━━━━━━━━━━━━━━━━\n⚠️ Lưu lại thông tin hoặc xem lại tại 📦 Đơn của tôi.")
    markup=types.InlineKeyboardMarkup(row_width=2); markup.add(types.InlineKeyboardButton("🛒 Mua thêm",callback_data="SHOP"),types.InlineKeyboardButton("📦 Đơn của tôi",callback_data="MY_ORDERS"))
    try: bot.edit_message_text(success_text,call.message.chat.id,call.message.message_id,parse_mode="HTML",reply_markup=markup)
    except Exception:
        try: bot.send_message(uid,success_text,parse_mode="HTML",reply_markup=markup)
        except Exception: pass
...
def cb_my_orders(call):
    ...
    with _lock:
        d=load(); my=[o for o in d.get("orders",[]) if str(o.get("uid"))==uid]
    if not my:
        bot.answer_callback_query(call.id,"Bạn chưa có đơn hàng nào.",show_alert=True); return
    lines=["<b>📦 LỊCH SỬ MUA HÀNG</b>","━━━━━━━━━━━━━━━━━━"]
    for o in my[-5:]:
        if o.get("key"):
            lines.append(f"\n🧾 Đơn: <code>{html.escape(str(o.get('id','')))}</code>\n🔑 Key: <code>{html.escape(str(o.get('key','')))}</code>\n🎮 {html.escape(str(o.get('product','')))}\n💵 {fmt(int(o.get('price',0)))}\n📅 {html.escape(str(o.get('expires_at','N/A')).replace('T',' '))}")
        else:
            lines.append(f"\n🧾 Đơn: <code>{html.escape(str(o.get('id','')))}</code>\n🎮 {html.escape(str(o.get('product','')))}\n💵 {fmt(int(o.get('price',0)))}")
    text="\n".join(lines); kb=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🏠 Menu",callback_data="MAIN"))
